...
- [ ] Tap the **📲 WhatsApp Rx** button. Confirm it attempts to open WhatsApp with a prescription message (you don't need to actually send it).
...
## 6. Backup and recovery — the one thing that matters most if something goes wrong
...
## 7. Things that are known limitations right now (not bugs — don't report these, but do confirm you understand them)
...
- Settings → Cloud Backup is desktop-only by design in the current test coverage (it still works on mobile browsers, but automated regression testing for it currently only runs on desktop).
